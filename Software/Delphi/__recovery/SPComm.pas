// CommunicationsҐчЎC
//
// David Wann.COMM32.PAS Version 1.0ЎC¦p¤UЎG
...
type
  ...
  public
    ...
    // --------------------------------------------------------------------------------
    FInBufferSize: Word; // КдИл»єіеЗшґуРЎ
    FInputLen: Word; // ГїґОґУ»єіеЗшИЎЧЦЅЪКэ
    // --------------------------------------------------------------------------------
  ...
  private
    ...
    // --------------------------------------------------------------------------------
    FPortOpen: Boolean; // Ч·јУ¶ЛїЪїШЦЖЧЦ¶О
    FCommPort: BYTE; // Ч·јУ¶ЛїЪєЕЧЦ¶О
    FPortOpenError: String; // Ч·јУ¶ЛїЪґтїЄґнОуЧЦ¶О
    FOutput: AnsiString; // Ч·јУ¶ЛїЪКдіцЧЦ·ыЧЦ¶О
    FInBufferSize: Word; // КдИл»єіеЗшґуРЎ
    FInputLen: Word; // ГїґОґУ»єіеЗшИЎЧЦЅЪКэ
    // --------------------------------------------------------------------------------
    FCommName: String; // ¶ЛїЪГыЧЦ¶О,MSCommОЄ¶ЛїЪєЕCommPort
    FBaudRate: DWORD; // ІЁМШВКЧЦ¶О,MSCommФЪSettingsДЪ
    FParityCheck: Boolean; // РЈСйО»ЧЦ¶О,MSCommФЪSettingsДЪ
    ...
    // --------------------------------------------------------------------------------
    procedure SetPortOpen(b: Boolean); // ґтїЄ¶ЛїЪ
    function GetPortOpen: Boolean; // ґтїЄ¶ЛїЪ
    procedure SetCommPort(CommPort: BYTE); // ЙиЦГ¶ЛїЪєЕ
...
const
  ...
  // --------------------------------------------------------------------------------
  // ·ПіэЈ¬УГInBufferSizeКфРФМжґъЈ¬ФцЗїБй»оРФ
...
begin
  ...
  // А¶САґ®їЪїЙДЬ±»К¶±рОЄUnknownЈ¬ОЄБЛјжИЭА¶САґ®їЪФцјУБЛ FILE_TYPE_UNKNOWN
...
begin
  if Assigned(FOnReceiveData) then // Из№ыМнјУБЛУГ»§ґ®їЪКВјюґ¦АнФтЦґРР
    FOnReceiveData(self, Buffer, BufferLength) // УГ»§ґ®їЪКВјюґ¦Ан
end;
...
procedure TComm.SetPortOpen(b: Boolean); // ґтїЄ¶ЛїЪ
begin
  if b = True then
    StartComm // ґтїЄ¶ЛїЪ
  else
...
function TComm.GetPortOpen: Boolean; // ґтїЄ¶ЛїЪ
begin
  if hCommFile = 0 then
  begin
    FPortOpen := False;
...
procedure TReadThread.Execute;
var
  szInputBuffer: array of Char; // РВ·Ѕ·ЁБй»о
  // szInputBuffer: array[0..INPUTBUFFERSIZE-1] of Char;//АП·Ѕ·ЁІ»Бй»о
...
label
  EndReadThread;
begin
  SetLength(szInputBuffer, FInBufferSize); // РВ·Ѕ·ЁКэЧйґуРЎїЙ±д
  ...
  if not SetupReadEvent(@overlappedRead,
    // PAnsiChar(szInputBuffer),//РВ·Ѕ·ЁІ»ДЬУГ@szInputBuffer
    PAnsiChar(szInputBuffer), // РВ·Ѕ·ЁІ»ДЬУГ@szInputBuffer
